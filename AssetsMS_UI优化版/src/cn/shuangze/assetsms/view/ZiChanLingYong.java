 ...
 * @author yuanshuai
 */
public class ZiChanLingYong extends JFrame implements ActionListener, ListSelectionListener{
    ...
    public void Findinit() throws Exception {
       try{
        ...
        //中部面板的布局List<AssetsTrjn> findByFromAcc(String fromacc) throws Exception
         java.util.List<AssetsTrjn> list1 = assetsTrjnDao.findByFromAcc("领用");
          String[][] colValue = null;
        int size = list1.size();
        if (!list1.isEmpty()) {
            colValue = new String[list1.size()][7];
        }
        for (int i = 0; i < size; i++) {
            colValue[i][0] = Integer.toString(list1.get(i).getJourNo());
            colValue[i][1] = list1.get(i).getFromAcc();
            colValue[i][2] = Integer.toString(list1.get(i).getAssetsID());
            colValue[i][3] = list1.get(i).getRegDate().toString();
            colValue[i][4] = Integer.toString(list1.get(i).getPersonID());
            colValue[i][5] = list1.get(i).getPurpose();
            colValue[i][6] = list1.get(i).getOther();
        }
        if (colValue == null) {
    ...
    private void init() throws Exception {
        ...
        this.setLocation((screenSize.width - 1600) / 2 + 45,
                (screenSize.height - 300) / 2 + 45);
        //东部面板的布局 
        String[][] colValue1 = assetsDao.findByStatus("可用");
        if (colValue1 == null) {
            colValue1 = new String[1][8];
        }
        String[] colName1 = {"资产编号", "资产名称", "资产类别编号","型号","价格","购买日期","状态","备注"};
        tableModel2= new DefaultTableModel(colValue1, colName1);
        ...
        String[][] colValue = assetsTrjnDao.findAll();
        if (colValue == null) {
            colValue = new String[1][7];
        }
        String[] colName = {"编号", "操作类型", "资产编号","操作时间","领用人编号","用途","备注"};
        ...
        if (obj == receive) {
            try {
                ...
                if(jTextField4.getText().equals(""))
                    JOptionPane.showMessageDialog(null, "请输入日期！", "资产领用", JOptionPane.INFORMATION_MESSAGE);
                else{
                int j=0;
                ...
                Assets assets=assetsDao.findByAssetsID(Integer.parseInt(jTextField3.getText()));
                    assetsTrjnDao.add(new AssetsTrjn(jTextField2.getText(),Integer.parseInt(jTextField3.getText()),java.sql.Date.valueOf(jTextField4.getText()),j,jTextField6.getText(),jTextField7.getText()));
                     assetsDao.modify(new Assets(assets.getAssetsID(),assets.getName(),assets.getTypeID(),assets.getModel(),assets.getPrice(),assets.getBuyDate(),"领用",assets.getOther()));
            ...
            this.dispose();
            ZiChanLingYong typeMan = new ZiChanLingYong();
            typeMan.downInit();
            typeMan.pack();
