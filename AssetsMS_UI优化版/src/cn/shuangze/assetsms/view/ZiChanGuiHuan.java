 ...
 * @author yuanshuai
 */
public class ZiChanGuiHuan extends JFrame implements ActionListener, ListSelectionListener{
    ...
    public void Findinit() throws Exception {
       try{
        ...
        //中部面板的布局List<AssetsTrjn> findByFromAcc(String fromacc) throws Exception
         java.util.List<AssetsTrjn> list = assetsTrjnDao.findByFromAcc("归还");
    ...
    private void init() throws Exception {
        ...
        //设置运行时窗口的位置
        Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
        this.setLocation((screenSize.width - 1600) / 2 + 25,
                (screenSize.height - 300) / 2 + 45);
        //中部面板的布局 
        String[][] colValue2 = assetsDao.findByStatus("领用");
        if (colValue2 == null) {
    ...
    public void downInit() {
        ...
        give_back.setText("归还");
        give_back.setFont(new Font("Dialog", Font.BOLD, 12));
        downPanel.add(give_back);
        ...
        if (obj == give_back) {
            try {
                ...
                Assets assets=assetsDao.findByAssetsID(Integer.parseInt(jTextField3.getText()));
                    ...
                    assetsDao.modify(new Assets(assets.getAssetsID(),assets.getName(),assets.getTypeID(),assets.getModel(),assets.getPrice(),assets.getBuyDate(),"可用",assets.getOther()));
            ...
            this.dispose();
            ZiChanGuiHuan typeMan = new ZiChanGuiHuan();
            typeMan.downInit();
            typeMan.pack();
