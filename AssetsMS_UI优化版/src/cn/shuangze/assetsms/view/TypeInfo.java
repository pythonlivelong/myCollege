...
public class TypeInfo extends JFrame implements ActionListener, ListSelectionListener {
    ...
    private void init() throws Exception {
        ...
        this.setLocation((screenSize.width - 800) / 2 + 65,
        ...
        if (obj == addInfo) {
            ...
            if (rowcount >= 0) {
                try {
                    ...
                    if (i == 0) {
                        java.util.List<Assets> list ;
                        java.util.List<AssetsTrjn> list1;
                                list=assetsDao.findByTypeID(Integer.parseInt(jTextField1.getText()));
                                
                                for(int j=0;j<list.size();j++)
                                {
                                    list1=assetsTrjnDao.findByAssets(list.get(j).getAssetsID());
                                    for(int k=0;k<list1.size();k++)
                                    {
                                        assetsTrjnDao.delete(list1.get(k).getJourNo());
                                    }
                                    assetsDao.delete(list.get(j).getAssetsID());
                                }
